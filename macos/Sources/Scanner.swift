...
enum Scanner {
    ...
    static func scanLocalDevice(showAll: Bool = false) async -> Device {
        ...
        let services = await fetchServices(
            host: "127.0.0.1",
            ports: ports.filter { $0 != MANIFEST_PORT },
            hints: projects,
            os: "darwin",
            showAll: showAll
        )
    ...
    static func fetchServices(host: String, ports: [Int], hints: [Int: String], os: String? = nil, showAll: Bool = false) async -> [WebService] {
        ...
        return await withTaskGroup(of: WebService?.self) { group in
            ...
            // Deduplicate HTTP/HTTPS pairs on same host with same title
    ...
    static func fetchTitle(host: String, port: Int) async -> WebService? {
        ...
        for scheme in schemes {
            ...
            case .redirectedToPort(_):
                // This port redirects to a different port, skip it
                return nil
    ...
    static func httpTitle(url: URL) async -> FetchResult {
        ...
        do {
            ...
            // Check if we were redirected to a different port
...
// Accepts self-signed certs and detects cross-port redirects
